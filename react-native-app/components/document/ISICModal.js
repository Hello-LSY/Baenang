      ...
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={styles.modalContainer} onPress={(e) => e.stopPropagation()}>
          ...
          <View style={styles.topColor}></View>
            ...
            {/* 모달 타이틀 - 위치 고정 */}
            <Text style={styles.modalTitle}>{'국제학생증'}</Text>
            <Text style={styles.modalTitleEng}>{"ISIC Card"}</Text>
                  ...
                  <View style={styles.infoContainer}>
                    {isDetailView ? (
                      <View>
                        ...
                        <Text style={styles.textDetailInfo}>{isic.name}</Text>
                        <Text style={styles.textLabel}>학교명</Text>
                        <Text style={styles.textDetailInfo}>{isic.schoolName}</Text>
                        <Text style={styles.textLabel}>국제학생증 번호</Text>
                        <Text style={styles.textDetailInfo}>{isic.isic}</Text>
                        <Text style={styles.textLabel}>인증기간</Text>
                      ...
                      <View style={styles.dataContainer}>
                        ...
                        {isic.imagePath && (
                          <View style={styles.imageContainer}>
                            <Image
                              source={{ uri: `${S3_URL}/uploads/${isic.imagePath}` }}
                              style={styles.image}
                            />
                          </View>
                        )}
                        {/* 정보 표시 */}
                        <Text style={styles.textName}>{isic.name}</Text>
                        <Text style={styles.textLabel}>국제학생증 번호</Text>
                        <Text style={styles.textInfo}>{isic.isic}</Text>
                      </View>
                    )}
                  </View>
                ) : (
            ...
            {/* Issuer - 하단 고정 */}
            <Text style={styles.textIssuer}>{isic ? isic.schoolName : ''}</Text>
          </Animated.View>
        </Pressable>
      </Pressable>
    </Modal>
